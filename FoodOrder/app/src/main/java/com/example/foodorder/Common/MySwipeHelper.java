...
import com.example.foodorder.Callback.MyButtonClickListener;
...
public abstract class MySwipeHelper extends ItemTouchHelper.SimpleCallback {
    ...
    public class MyButton {
        ...
        public void onDraw(Canvas c, RectF rectF, int pos) {
            ...
            float x = 0, y = 0;
            if (imageResId == 0) //If Just, Show Text ONly
            {
    ...
    public void onChildDraw(@NonNull Canvas c, @NonNull RecyclerView recyclerView, @NonNull RecyclerView.ViewHolder viewHolder, float dX, float dY, int actionState, boolean isCurrentlyActive) {
        ...
        if(actionState == ItemTouchHelper.ACTION_STATE_SWIPE){
            if(dX < 0)
            {
                ...
                transalationX = dX*buffer.size()*buttonWidth/itemView.getWidth();
                drawBuxtton(c, itemView, buffer, pos, transalationX);

            }
        }

        super.onChildDraw(c, recyclerView, viewHolder,transalationX, dY, actionState, isCurrentlyActive);
    }


    private void drawBuxtton(Canvas c, View itemView, List<MyButton> buffer, int pos, float transalationX) {
        float right = itemView.getRight();
        float dButtonWidth = -1*transalationX/buffer.size();
