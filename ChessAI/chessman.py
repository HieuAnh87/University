...
def cal_rect(bool, rect0, rect1):
    if bool:
        return (rect1 * 50, rect0 * 50)
    else:
        return (rect1 * 50 + 50//2, rect0 * 50 + 50//2)
...
class Move():
    ...
    def update(self, name, p1, p2):
        self.m.append((name, p1, p2))
        self.len += 1

    def precond_en_passant(self, ar, a, b, type):
        lst = []
        if type == 'b' and a == 4:
        ...
        for i in range(8):
            for j in range(8):
                if ar[i][j] != '  ' and ar[i][j][0] != type:
                    for pos in P[ar[i][j]].move(ar, (i+1,j+1), ar[i][j][0]):
                        if pos[0] == x and pos[1] == y:
                            return True
        return False

    def is_prev_move(self, chessman1, p2, chessman2, b1, b2):
        len = self.len - 1
        if self.m != []:
            while len >= 0:
                if self.m[len][0] == chessman1 and self.m[len][2] == p2:
                    break
                if self.m[len][0] == chessman2 and self.m[len][2] == b2 and self.m[len][1] == b1:
                    return True
                len -= 1
        return False
    ...
    def is_chessman_moved(self, chessman):
        if self.m != []:
            for i in self.m:
                if i[0] == chessman:
                    return True
...
class Pieces():
    def __init__(self, screen):
        self.screen = screen
        self.prev_move = Move()
        self.init_array_chessman()
        self.init_surf_chessman()

    def init_array_chessman(self):
    ...
    def draw_chessman_upgrade(self, pos):
        for i in range(8):
            for j in range(8):
                if pos != () and i == pos[0][0]-1 and j == pos[0][1]-1:
                    pygame.draw.rect(
                            self.screen, (255, 102, 102),
                            (50*(j+1), 50*(i+1), 50, 50))
                if self.ar[i][j] != '  ':
                    if (i + j) % 2 == 0:
                        CLR = (192, 192, 192)
    ...
    def available_moves(self, pc, p, type):
        return self.P[pc].move(self.ar, p, type)
    ...
    def precond_castling(self, ar, type):
        res = [0,0]
        if not self.is_checked(ar, type) and not self.prev_move.is_chessman_moved(type+'k'):
            p = king_position(ar, type)
            x, y = p[0], p[1]
