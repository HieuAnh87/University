...
from setup import *
from classes_chessman import *

class AI():
    def __init__(self, ar, chessman):
        self.ar = ar
    ...
    def lst_chessman_available(self, ar, type):
        lst_chessman = []
        for i in range(8):
            for j in range(8):
                if ar[i][j][0] == type:
                    lst_chessman.append((ar[i][j], i, j))
        return lst_chessman
        ...
        for i in range(8):
            for j in range(8):
                if ar[i][j] != '  ' and ar[i][j][0] != type:
                    for pos in self.P[ar[i][j]].move(ar,(i+1,j+1),ar[i][j][0]):
    ...
    def is_checkmate_AI_Move(self, ar, type, prev_move):
        if self.is_checked_AI_Move(ar, type):
            for x in range(8):
                for y in range(8):
                    if ar[x][y][0] == type:
                        lst = self.P[ar[x][y]].move(ar,(x+1,y+1),type)
                        if ar[x][y][-1] == 'p':
    ...
    def selecting_AI_Move(self, ar, p, prev_move):
        x, y = p[0]-1, p[1]-1
        type = ar[x][y][0]
        lst = self.P[ar[x][y]].move(ar, p, type)
        lst_ai = []
        if ar[x][y][-1] == 'k':
...
class AI_Minimax(AI):
    def __init__(self, ar, chessman):
        super().__init__(ar,chessman)

    def minimax(self,ar,chessman,type,alpha,beta,depth, last_move,prev_move):
        if depth == 0:
            return [self.eval_board(ar),last_move[0],last_move[1]]
        draw = True
        if type == 'b':
            max_s = [-1000000000, None, None]
            ps = self.lst_chessman_available(ar, type)
            random.shuffle(ps)
            for chessman in ps:
                x, y = chessman[1], chessman[2]
                ...
                for pos in lst_ai:
                    ...
                    if self.is_checkmate_AI_Move(cp_ar, 'w',prev_move):
                        ...
                        score = self.minimax(cp_ar,chessman,'w',alpha,beta,depth-1, ((x+1,y+1),pos),cp_prev_move)
                        score[0] += Score[img][pos[0]-1][pos[1]-1]
                    if score[0] >= max_s[0]:
            ...
            for chessman in ps:
                ...
                for pos in lst_ai:
                    ...
                    if self.is_checkmate_AI_Move(cp_ar, 'b', prev_move):
                        ...
                        score = self.minimax(cp_ar,chessman,'b',alpha,beta,depth-1, ((x+1,y+1),pos),cp_prev_move)
                        score[0] += Score[img][pos[0]-1][pos[1]-1]
                    if score[0] <= min_s[0]:
...
class AI_rand(AI):
    ...
    def find_pos_random(self, ar, chessman, type):
        lst_chessman = self.lst_chessman_available(ar, type)
        random.shuffle(lst_chessman)
        for p in lst_chessman:
            lst_ai = chessman.selecting_AI((p[1]+1, p[2]+1))
            if lst_ai == []:
                continue
